...
import { isEmptyObject, isString } from '@kdu-i18n/shared'
import { generateJSON, generateYAML } from '@kdu-i18n/bundle-utils'

import type { CodeGenOptions, DevEnv } from '@kdu-i18n/bundle-utils'
import type { KduI18nLoaderOptions } from './options'
...
const loader: webpack.loader.Loader = function (
  ...
  try {
    ...
    this.emitError(`[kdu-i18n-loader]: ${err.message}`)
  }
}
...
function getOptions(
  ...
  const { forceStringify, productionSourceMap } =
    loaderOptions as KduI18nLoaderOptions
  ...
  const baseOptions = {
    ...
    onWarn: (msg: string): void => {
      loaderContext.emitWarning(
        `[kdu-i18n-loader]: ${loaderContext.resourcePath} ${msg}`
      )
    },
