...
import { isArray, isBoolean, isEmptyObject, isString } from '@kdu-i18n/shared'
import { createFilter } from '@rollup/pluginutils'
import {
  generateJSON,
  generateYAML,
  checkInstallPackage
} from '@kdu-i18n/bundle-utils'
import fg from 'fast-glob'
import { debug as Debug } from 'debug'
import { parseKduRequest } from './query'
import { normalizePath } from 'wite'

import type { Plugin, ResolvedConfig, UserConfig } from 'wite'
import type { CodeGenOptions, DevEnv } from '@kdu-i18n/bundle-utils'
import type { WitePluginKduI18nOptions } from './options'

const debug = Debug('wite-plugin-kdu-i18n')

const KDU_I18N_BUNDLE_IMPORT_ID = '@kdu-i18n/wite-plugin-kdu-i18n/messages'

const installedPkg = checkInstallPackage(
  '@kdu-i18n/wite-plugin-kdu-i18n',
  debug
)

function pluginI18n(
  options: WitePluginKduI18nOptions = { forceStringify: false }
): Plugin {
  ...
  const runtimeOnly = isBoolean(options.runtimeOnly)
    ...
    : true
  // prettier-ignore
  const compositionOnly = installedPkg === 'kdu-i18n'
    ? isBoolean(options.compositionOnly)
      ? options.compositionOnly
      : true
    : true
  // prettier-ignore
  const fullIinstall = installedPkg === 'kdu-i18n'
  ...
  const globalSFCScope = isBoolean(options.globalSFCScope)
    ...
    : false
  const useKduI18nImportName = options.useKduI18nImportName
  if (useKduI18nImportName != null) {
    console.warn(
      `[wite-plugin-kdu-i18n]: 'useKduI18nImportName' option is experimental`
    )
  }
  const getAliasName = () =>
    installedPkg === 'petite-kdu-i18n' &&
    isBoolean(useKduI18nImportName) &&
    useKduI18nImportName
      ? 'kdu-i18n'
      : `${installedPkg}`
  let config: ResolvedConfig | null = null

  return {
    name: 'wite-plugin-kdu-i18n',
    ...
    config(config: UserConfig, { command }) {
      if (command === 'build' && runtimeOnly) {
        ...
        if (isArray(config.resolve!.alias)) {
          config.resolve!.alias.push({
            find: 'kdu-i18n',
            replacement: `petite-kdu-i18n/dist/petite-kdu-i18n.esm-bundler.js`
          })
        } else {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          ;(config.resolve!.alias as any)[
            'kdu-i18n'
          ] = `petite-kdu-i18n/dist/petite-kdu-i18n.esm-bundler.js`
        }
        debug(`alias name: ${getAliasName()}`)
      }

      config.define = config.define || {}
      config.define['__KDU_I18N_LEGACY_API__'] = !compositionOnly
      debug(
        `set __KDU_I18N_LEGACY_API__ is '${config.define['__KDU_I18N_LEGACY_API__']}'`
      )

      config.define['__KDU_I18N_FULL_INSTALL__'] = fullIinstall
      debug(
        `set __KDU_I18N_FULL_INSTALL__ is '${config.define['__KDU_I18N_FULL_INSTALL__']}'`
      )

      config.define['__KDU_I18N_PROD_DEVTOOLS__'] = false
    },
    ...
    configResolved(_config: ResolvedConfig) {
      ...
      const jsonPlugin = config.plugins.find(p => p.name === 'wite:json')
      if (jsonPlugin) {
    ...
    resolveId(id: string) {
      if (id === KDU_I18N_BUNDLE_IMPORT_ID) {
        return id
      }
    },
    ...
    async load(id: string, ssr: boolean) {
      if (id === KDU_I18N_BUNDLE_IMPORT_ID && include) {
    ...
    async transform(code: string, id: string) {
      ...
      if (!query.kdu) {
        if (/\.(json5?|ya?ml)$/.test(id) && filter(id)) {
          ...
          // NOTE:
          // `.json` is handled default in wite, and it's transformed to JS object.
          let _source = code
          if (langInfo === '.json') {
...
function getOptions(
  ...
  const baseOptions = {
    ...
    onWarn: (msg: string): void => {
      console.warn(`[wite-plugin-kdu-i18n]: ${filename} ${msg}`)
    },
    onError: (msg: string): void => {
      console.error(`[wite-plugin-kdu-i18n]: ${filename} ${msg}`)
    }
  }
...
export const kduI18n = pluginI18n
